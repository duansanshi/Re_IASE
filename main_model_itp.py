...
class PriSTI(nn.Module):
    ...
    def init_student(self):
        self.student_n_samples = 10
        self.student_imputer = OneStep_Model(self.config,self.device)
        self.student_imputer.load_state_dict(torch.load("/home/duanlei/PriSTI/student.pth",map_location=self.device))
    ...
    def forward(self, batch, is_train=1):
        ...
        coeffs_info = coeffs.unsqueeze(1)
        itp_info = self.get_itp_info(observed_data,cond_mask,side_info,n_samples=self.student_n_samples,coeffs_info=coeffs_info)
    ...
    def get_itp_info(self,observed_data,cond_mask,side_info,n_samples,coeffs_info):
        imputed_samples = self.student_imputer.real_impute(observed_data, cond_mask, side_info, n_samples, itp_info=coeffs_info) #(B,n,K,L)
        samples_median = imputed_samples.median(dim=1)
        return samples_median.values #(B,K,L)
