...
class NoiseProject(nn.Module):
    def __init__(self, side_dim, channels, diffusion_embedding_dim, nheads, target_dim, proj_t, order=2, include_self=True,
        ...
        #for revin
        self.revin_time = RevIN(num_features=target_dim*36)
        self.revin_feature = RevIN(num_features=target_dim*36)

        # self.revin_time = RevIN(num_features=target_dim)
        # self.revin_feature = RevIN(num_features=target_dim)
    ...
    def forward(self, x, side_info, diffusion_emb, itp_info, support):
        ...
        # y = self.revin_time(y,mode='norm')
        # y = self.forward_time(y, base_shape, itp_info)
        # y = self.revin_time(y,mode='denorm')

        # y = self.revin_feature(y,mode='norm')
        # y = self.forward_feature(y, base_shape, support, itp_info)  # (B,channel,K*L)
        # y = self.revin_feature(y,mode='denorm')
