...
class PriSTI(nn.Module):
    def __init__(self, target_dim, seq_len, config, device):
        ...
        self.revin_layer = RevIN(num_features=target_dim).to(device)
    ...
    def forward(self, batch, is_train=1):
        ...
        observed_data = self.revin_layer(observed_data,mode='norm') #(B,L,K) 
        observed_data.permute(0,2,1)  #(B,L,K)
        side_info = self.get_side_info(observed_tp, cond_mask)
        itp_info = None
        if self.use_guide:
            coeffs = coeffs.permute(0,2,1)  #(B,L,K)
            coeffs = self.revin_layer(coeffs,mode='norm') #(B,L,K) 
            coeffs = coeffs.permute(0,2,1)  #(B,K,L)
