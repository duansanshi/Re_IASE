...
class GuideDiffLightning(pl.LightningModule):
    ...
    def __init__(self, model_config, learning_rate=5e-4, final_loss_weight=1.0,epochs=200, decay_flag = False):
        ...
        self.epochs = epochs
        # 实例化模型
        self.model = Guide_diff(config=model_config)
        ...
        self.embed_layer = nn.Embedding(
            num_embeddings=36, embedding_dim=16
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        final_pred, layer_preds = self(total_input, torch.tensor([99]).to(self.device), side_info, coeffs.unsqueeze(1), cond_mask)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # # 4. 计算中间层预测损失 (Layer-wise Loss)
        # layer_loss_list = []
        # for pred in layer_preds:
        #     # pred 的形状也是 (B, K, L)，直接与 ground_truth 对齐
        #     layer_loss = self.criterion(pred*target_mask, observed_data*target_mask)
        #     layer_loss_list.append(layer_loss)
        
        # # 计算所有中间层损失的平均值
        # if layer_loss_list:
        #     layer_mean_loss = torch.stack(layer_loss_list).mean()
        # else:
        #     # 只有一层时或 layer_preds 为空时，layer_mean_loss 为 0
        #     layer_mean_loss = torch.tensor(0.0, device=self.device)

        # 5. 计算总损失 (Total Loss)
        # total_loss = (self.final_loss_weight * final_loss) + \
        #              (self.layer_loss_weight * layer_mean_loss)
    ...
    def test_step(self,batch,batch_idx):
        ...
        with torch.no_grad():
            ...
            t1 = time.time()
            final_pred, layer_preds = self.model(diff_input,torch.tensor([99]).to(self.device),side_info,itp_info,cond_mask)
            sample = final_pred
    ...
    def get_side_info(self, observed_tp, cond_mask):
        ...
        feature_embed = self.embed_layer(
            torch.arange(36).to(self.device)
        )  # (K,emb)
