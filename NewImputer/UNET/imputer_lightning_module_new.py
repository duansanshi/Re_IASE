        ...
        if self.loss_flag == "l1":
            ...
            if num_layers > 0:
                ...
                for i, pred in enumerate(layer_preds):
                    current_weight = current_weights[i]  # weighted coefficient for each layer
                    
                    # Apply weighted error for all layers
                    abs_error = torch.abs(observed_data - pred)
                    masked_error = abs_error * eval_mask
                    
                    # Apply decay only for intermediate layers (not final layer)
                    if i < num_layers - 1:  
                ...
                # Sum all weighted losses and normalize once
            ...
            if num_layers > 0:
                # ====================== 改进的中间监督策略 ======================
                # 改进1：修复权重分配逻辑
                # 前面的层权重更小（0.2），后面的层权重更大（1.0），确保梯度逐层加强
                weights = torch.linspace(0.2, 1.0, num_layers, dtype=torch.float, device=self.device)
                
                # 改进2：添加可配置的warm-up策略
                if self.decay_flag:
                    # 前33%的epoch进行warm-up，使中间监督强度逐渐增加
                    warmup_factor = min(1.0, self.current_epoch / (self.epochs * 0.33))
                else:
                    warmup_factor = 1.0

                weighted_layer_losses = []
                total_abs_error = 0  # 用于记录各层的平均绝对误差
                
                for i, pred in enumerate(layer_preds):
                    # 计算当前层的 L1 损失（注意这里不做层级归一化）
                    abs_error = torch.abs(observed_data - pred)
                    masked_error = abs_error * eval_mask
                    
                    # 应用权重和 warm-up（但在求和时不再做第二次归一化）
                    current_weight = weights[i] * warmup_factor
                    weighted_layer_losses.append(current_weight * masked_error.sum())
                    
                    # 记录原始信息用于调试
                    if i == num_layers - 1:  # 只在最后一层记录
