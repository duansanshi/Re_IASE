...
class AQI36_Dataset(Dataset):
    def __init__(self, eval_length=36, target_dim=36, mode="train", val_len=0.1, is_interpolate=False,
                 target_strategy='hybrid', mask_sensor=None, missing_ratio=None, preimpute_flag="Forward", freq_flag=False,
                 freq_ratio=0.1, **kwargs):
    ...
    def __getitem__(self, org_index):
        ...
        eval_mask = ob_mask - cond_mask.numpy()
        eval_mask = np.clip(eval_mask, 0, 1).astype(np.float32)

        easy_mask, medium_mask, hard_mask, seg_len = self.build_difficulty_masks_2d(eval_mask)

        layer_mask_1 = easy_mask
        layer_mask_2 = ((easy_mask + medium_mask) > 0).astype(np.float32)
        layer_mask_3 = eval_mask.astype(np.float32)
        s = {
            ...
            "layer_mask_1": layer_mask_1,
            "layer_mask_2": layer_mask_2,
            "layer_mask_3": layer_mask_3,
        }
...
####################################################################################
            ...
            if freq_flag == True and preimpute_flag != "None":
    ...
    def build_difficulty_masks_2d(self, target_mask):
        """
        target_mask: [L, K]
        1 表示当前需要被该层监督的位置（通常传 eval_mask = ob_mask - cond_mask）
        按连续缺失段长度划分 easy / medium / hard
        """
        L, K = target_mask.shape
        seg_len = np.zeros_like(target_mask, dtype=np.int32)

        for k in range(K):
            t = 0
            while t < L:
                if target_mask[t, k] > 0.5:
                    start = t
                    while t < L and target_mask[t, k] > 0.5:
                        t += 1
                    length = t - start
                    seg_len[start:t, k] = length
                else:
                    t += 1

        easy_mask = ((seg_len > 0) & (seg_len <= 2)).astype(np.float32)
        medium_mask = ((seg_len >= 3) & (seg_len <= 5)).astype(np.float32)
        hard_mask = (seg_len >= 6).astype(np.float32)

        return easy_mask, medium_mask, hard_mask, seg_len


def get_dataloader(batch_size, device, val_len=0.1, is_interpolate=False, num_workers=4, target_strategy='hybrid', mask_sensor=None, preimpute_flag="Forward", freq_flag=False, freq_ratio=0.1, **kwargs):
    dataset = AQI36_Dataset(mode="train", is_interpolate=is_interpolate, target_strategy=target_strategy, mask_sensor=mask_sensor, preimpute_flag=preimpute_flag, freq_flag=freq_flag, freq_ratio=freq_ratio)
    train_loader = DataLoader(
        dataset, batch_size=batch_size, num_workers=num_workers, shuffle=True
    )
    dataset_test = AQI36_Dataset(mode="test", is_interpolate=is_interpolate, target_strategy=target_strategy, mask_sensor=mask_sensor, preimpute_flag=preimpute_flag, freq_flag=freq_flag, freq_ratio=freq_ratio)
    test_loader = DataLoader(
        dataset_test, batch_size=batch_size, num_workers=num_workers, shuffle=False
    )
    dataset_valid = AQI36_Dataset(mode="valid", val_len=val_len, is_interpolate=is_interpolate, target_strategy=target_strategy, mask_sensor=mask_sensor, preimpute_flag=preimpute_flag, freq_flag=freq_flag, freq_ratio=freq_ratio)
