"""
v9 Trainer: 可学习频率分解 (AdaptiveFreqDecomposition)
使用 v9 架构 (transformer_imputer_v9) + v5 lightning module
模型内部自动做 raw→trend+detail 分解，4通道输入 [raw, mask, trend, detail]
"""
...
import numpy as np
import argparse
from transformer_imputer_v9 import transformer_imputer
...
decomp_params = sum(p.numel() for p in model.decomposition.parameters())
print(f"Total params: {total_params:,} | Decomposition params: {decomp_params}")

if args.exp_name:
    exp_name = args.exp_name
else:
    exp_name = f"v9_learnfreq_t{args.curriculum_temperature}"
...
print(f"Architecture: v9 (AdaptiveFreqDecomposition + 4ch input)")
print(f"{'='*50}\n")
