...
parser.add_argument("--exp_name", type=str, default="p08_ablation")
...
pems08_dm = Pems08_DataModule(preimpute_flag=args.preimpute, freq_flag=freq_flag)
model = transformer_imputer(device=device)
imputer = pems08_lightning_module(model, lr=5e-4, loss_flag="l1", epochs=epochs, decay_flag=False)
...
trainer.fit(imputer, pems08_dm)
best_model_path = checkpoint_callback.best_model_path
print(f"\nLoading best model from {best_model_path}")
imputer = pems08_lightning_module.load_from_checkpoint(best_model_path, model=model, lr=5e-4)
trainer.test(imputer, pems08_dm)
