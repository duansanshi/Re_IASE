...
class transformer_imputer_cascade(nn.Module):
    def __init__(self, device, d_model=64, seq_len=36, feature_dim=36, pe_dim=128, use_dynamic_gating=False):
        super().__init__()
        self.device = device
        self.d_model = d_model
        self.seq_len = seq_len
    ...
    def _predict_increment(self, x):
        layer_delta = self.output_proj_1(x)
        layer_delta = F.relu(layer_delta)
        layer_delta = self.output_proj_2(layer_delta).squeeze(-1)
        return layer_delta
