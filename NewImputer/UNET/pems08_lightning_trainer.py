...
#     device=device,input_dim=170
# )

if train_flag:
    aqi36_imputer = pems08_lightning_module(model,lr=5e-4,loss_flag=loss_flag,epochs=epochs,decay_flag=Decay)
else:
    aqi36_imputer = pems08_lightning_module.load_from_checkpoint(checkpoint_path="/home/duanlei/PriSTI/checkpoints/best_model-v75.ckpt",model=model,lr=1e-4)
...
checkpoint_callback = ModelCheckpoint(
    monitor="val_loss",
    dirpath="checkpoints/baseline_pems08_freq",
    filename="best_model",
    save_top_k=1,
    mode="min",
    verbose=True,
...
if train_flag:
    trainer.fit(aqi36_imputer, pems08_dm)
    best_model_path = checkpoint_callback.best_model_path
    print(f"\nLoading best model from {best_model_path}")
    aqi36_imputer = pems08_lightning_module.load_from_checkpoint(best_model_path, model=model, lr=5e-4)
trainer.test(aqi36_imputer, pems08_dm)