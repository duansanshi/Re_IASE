...
parser.add_argument("--curriculum_temperature", type=float, default=0.5)
parser.add_argument("--smooth_weight", type=float, default=0.03)
parser.add_argument("--limit_train_batches", type=int, default=742)
parser.add_argument("--test_only", action="store_true", default=False)
parser.add_argument("--no_test", action="store_true", default=False)
parser.add_argument("--checkpoint_path", type=str, default=None)
parser.add_argument("--exp_name", type=str, default=None)
parser.add_argument("--preimpute", type=str, default="Backward")
...
pems04_dm = Pems04_DataModule(preimpute_flag=args.preimpute, freq_flag=not args.no_freq)
model = transformer_imputer(device=device)

if args.exp_name:
    exp_name = args.exp_name
else:
    exp_name = f"pems04_v5_t{args.curriculum_temperature}_c{args.curriculum_weight}"
    if args.smooth_weight > 0:
        exp_name += f"_s{args.smooth_weight}"

print(f"\n{'='*50}")
print(f"Experiment: {exp_name}")
print(f"Device: cuda:{device_num}, Epochs: {epochs}, LR: {args.lr}")
print(f"Curriculum: weight={args.curriculum_weight}, warmup={args.curriculum_warmup_ratio}, temperature={args.curriculum_temperature}")
print(f"Smooth: {args.smooth_weight}")
print(f"limit_train_batches: {args.limit_train_batches}")
print(f"{'='*50}\n")

if train_flag:
    pems04_imputer = pems04_lightning_module(
    ...
    pems04_imputer = pems04_lightning_module.load_from_checkpoint(
...
if train_flag:
    trainer.fit(pems04_imputer, pems04_dm)
    if test_flag:
        ...
        print("Running Test...")
        print("="*50)
        trainer.test(pems04_imputer, pems04_dm)
else:
    if test_flag:
        print("\n" + "="*50)
        print("Running Test...")
