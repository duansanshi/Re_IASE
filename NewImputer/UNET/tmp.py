...
class transformer_imputer(nn.Module):
    ...
    # --- 修改后：定义独立的投影头 ---
        self.heads = nn.ModuleList([
            # Head 1: 处理 Block 1 输出，可以设计得简单些
            nn.Sequential(
                nn.Linear(d_model, d_model // 2),
                nn.ReLU(),
                nn.Linear(d_model // 2, 1)
            ),
            # Head 2: 处理 Block 2 输出
            nn.Sequential(
                nn.Linear(d_model, d_model),
                nn.ReLU(),
                nn.Linear(d_model, 1)
            ),
            # Head 3: 处理 Block 3 输出（主输出层，可以更深）
            nn.Sequential(
                nn.Linear(d_model, d_model),
                nn.LayerNorm(d_model), # 加入 Norm 提升深层稳定性
                nn.ReLU(),
                nn.Linear(d_model, 1)
            )
        ])
    ...
    def forward(self, x, cond_mask, tod=None, dow=None):
        ...
        for i, block in enumerate(blocks):
            ...
            current_feat = current_feat + pe # 注入位置信息
            
            # 使用独立的 Head 进行投影
            # current_feat shape: (B, K, L, d_model)
            pred = self.heads[i](current_feat).squeeze(-1) # -> (B, K, L)
            skip_results.append(pred)

        # 最终输出计算
        # 方案 A: 依然使用 sqrt 缩放
        #final_x = torch.sum(torch.stack(skip_results), dim=0) / math.sqrt(len(skip_results))
        
        # 方案 B (推荐): 给不同层赋予不同的权重 (比如深层权重更大)
