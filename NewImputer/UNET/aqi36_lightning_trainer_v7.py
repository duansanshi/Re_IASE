"""
v7 Trainer: 方案B - 频率辅助输入（双编码器加法融合）
"""
from aqi36_lightning_datamodule import AQI36_DataModule
from imputer_lightning_module_v7 import aqi_lightning_module
...
import numpy as np
import argparse
from transformer_imputer_v7 import transformer_imputer
...
# 频率辅助输入
parser.add_argument("--aux_freq", action="store_true", default=False,
                    help="启用频率辅助输入（同时提供 raw + FFT 滤波）")
parser.add_argument("--aux_freq_ratio", type=float, default=0.1,
                    help="辅助频率滤波比例（截断高频的比例）")
# 控制
...
aqi36_dm = AQI36_DataModule(
    ...
    aux_freq=args.aux_freq, aux_freq_ratio=args.aux_freq_ratio
)
...
if args.exp_name:
    ...
    exp_name = f"v7_auxfreq{args.aux_freq_ratio}_c{args.curriculum_weight}_s{args.smooth_weight}"

print(f"\n{'='*50}")
print(f"Experiment: {exp_name}")
print(f"Device: cuda:{device_num}, Epochs: {epochs}, LR: {args.lr}")
print(f"Soft Curriculum: weight={args.curriculum_weight}, warmup={args.curriculum_warmup_ratio}, temp={args.curriculum_temperature}")
print(f"Smooth: {args.smooth_weight}")
print(f"Aux Freq: {args.aux_freq}, ratio={args.aux_freq_ratio}")
print(f"{'='*50}\n")
