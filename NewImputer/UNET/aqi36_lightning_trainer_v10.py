"""
v10 Trainer: Gap-Length Prompting (缺失长度感知提示)
使用 v10 架构 (transformer_imputer_v10) + v5 lightning module
模型内部从 cond_mask 自动计算 gap lengths，作为可学习提示嵌入输入
"""
...
import numpy as np
import argparse
from transformer_imputer_v10 import transformer_imputer
...
parser.add_argument("--gap_embed_dim", type=int, default=16,
                    help="Gap-length embedding dimension")
...
model = transformer_imputer(device=device, gap_embed_dim=args.gap_embed_dim)

total_params = sum(p.numel() for p in model.parameters())
gap_params = sum(p.numel() for p in model.gap_embedding.parameters())
print(f"Total params: {total_params:,} | Gap embedding params: {gap_params:,}")

if args.exp_name:
    exp_name = args.exp_name
else:
    exp_name = f"v10_gapprompt_d{args.gap_embed_dim}"
...
print(f"Architecture: v10 (Gap-Length Prompting, embed_dim={args.gap_embed_dim})")
print(f"{'='*50}\n")
