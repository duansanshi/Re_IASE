...
class Pems08_Dataset(Dataset):
    def __init__(self, eval_length=24, mode="train", val_len=0.1, test_len=0.2, missing_pattern='block',
                 is_interpolate=False, target_strategy='random', missing_ratio=None, preimpute_flag="Backward", freq_flag=True):
        ...
        if missing_pattern == 'block':
            ...
            if missing_ratio is not None:
                eval_mask = sample_mask(shape=(17856, 170), p=missing_ratio, p_noise=0.05, min_seq=12, max_seq=12 * 4,
                                        rng=self.rng)
        elif missing_pattern == 'point':
            eval_mask = sample_mask(shape=(17856, 170), p=0., p_noise=0.25, max_seq=12, min_seq=12 * 4, rng=self.rng)
...
def get_dataloader(batch_size, device, val_len=0.1, test_len=0.2, missing_pattern='block',
                   is_interpolate=False, num_workers=4, target_strategy='random', preimpute_flag="Backward", freq_flag=True):
    dataset = Pems08_Dataset(mode="train", val_len=val_len, test_len=test_len, missing_pattern=missing_pattern,
                             is_interpolate=is_interpolate, target_strategy=target_strategy, preimpute_flag=preimpute_flag, freq_flag=freq_flag)
