...
class transformer_imputer(nn.Module):
    def __init__(self, device="cuda:1", d_model=64, seq_len=36, feature_dim=36, pe_dim=128):
        super().__init__()
        self.device = device
    ...
    def forward(self, x, cond_mask,xt, diffusion_step):
        """
        x: observed_data * cond_mask (B, K, L)
        cond_mask: 缺失值掩码 (B, K, L)
        xt: 当前含噪数据 (B, K, L)
        diffusion_step: 扩散步骤 (B, )
        tod: 时间信息 (B, K, L)
        dow: 日期信息 (B, K, L)
        """
        B, K, L = x.shape
        ...
        x = torch.cat([
            ...
            xt.unsqueeze(-1),        # (B, K, L, 1)
        ], dim=-1)  # (B, K, L, 3)
        
        # 输入投影
        x = self.input_proj(x)  # (B, K, L, d_model)
        diffusion_embedding = self.diffusion_embedding(diffusion_step)  # (B, 128)
        diffusion_embedding = self.diffusion_projection(diffusion_embedding) # (B, d_model)
        x = x + diffusion_embedding.unsqueeze(1).unsqueeze(2)
