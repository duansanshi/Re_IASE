...
class MambaBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        Output: (L, B, D)
        """
        x = x.permute(1, 0, 2)  # Mamba需要(B, L, D)
        x = self.mamba(x)
        x = x.permute(1, 0, 2)  # 恢复(L, B, D)
        return x
...
class transformer_imputer(nn.Module):
    def __init__(self, device,d_model=128,seq_len=36,feature_dim=36,pe_dim=128):
        ...
        self.mamba_time = MambaBlock(d_model=d_model)
        self.mamba_feature = MambaBlock(d_model=d_model)
    ...
    def forward(self, x, cond_mask):
        ...
        x_trans = self.mamba_time(x)  # (L,B,d_model)
        ...
        #x_feat_trans = self.mamba_feature(x_feat)  # (K,B,d_model)
