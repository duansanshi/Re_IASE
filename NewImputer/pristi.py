...
class Guide_diff(nn.Module):
    def __init__(self, config, inputdim=1, target_dim=36, is_itp=False):
        ...
        self.embed_layer = nn.Embedding(
            num_embeddings=target_dim, embedding_dim=16
        )
        ...
        if config["adj_file"] in ['AQI36', 'aqi36']:
        ...
        elif config["adj_file"] in ['pems08', 'PEMS08']:
            self.adj = get_similarity_pems08(thr=0.1)
        elif config["adj_file"] in ['pems04', 'PEMS04']:
            self.adj = get_similarity_pems04(thr=0.1)
        else:
            raise ValueError(f"Unsupported adj_file: {config['adj_file']}")
