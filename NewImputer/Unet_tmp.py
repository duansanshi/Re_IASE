...
class UnetBlock(nn.Module):
    ...
    def forward(self,x):
        ...
        # --- 下采样路径 ---
        # 第一级下采样
        x = x.reshape(B, K, L//4, 4*self.d_model)  # (B, K, L/3, 3*d_model)
        x = self.dp_proj1(x)  # (B, K, L/4, 2*d_model)
        skip1 = x.clone()
        
        # 第二级下采样
        x = x.reshape(B, K, L//12, 6*self.d_model)  # (B, K, L/12, 8*d_model)
        ...
        # --- 上采样路径 ---
        # 第二级上采样
        x = self.up_proj2(x)  # (B, K, L/12,6*d_model)
        x = x.reshape(B, K, L//4, 2*self.d_model)  # (B, K, L/3, 2*d_model)
        
        # 第二级Transformer
        x = x.reshape(B*K, L//4, -1)  # (B*K, L/3, 2*d_model)
        ...
        # 第一级上采样
        x = self.up_proj1(x)  # (B, K, L/4, 4*d_model)
