...
class MetrlaLightningModule(pl.LightningModule):
    def __init__(self,config, device, target_dim=207,seq_len=24):
        super(MetrlaLightningModule, self).__init__()
        self.model = OneStep_Model(config,device,target_dim,seq_len)
        #self.model.load_state_dict(torch.load("/home/duanlei/PriSTI/save/metr_la/model.pth"))
        self.step_counter = 0
...
#这一datamodule得到的每个batch是一个长度为10(times)的list，其中list[i]是一个字典，每个元素的尺寸为[B,4,36,36] [B,4,K,L]的顺序
    def training_step(self, batch, batch_idx):
        (
            ...
            coeffs,
            cond_mask,
        ) = self.model.process_data_metrla(batch)
