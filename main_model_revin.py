...
class PriSTI(nn.Module):
    def __init__(self, target_dim, seq_len, config, device):
        ...
        self.revin_layer = RevIN_REF(num_features=target_dim,affine=True).to(device)
    ...
    def calc_loss_valid(
        self, observed_data, cond_mask, observed_mask, side_info, itp_info, is_train,revin_ref
    ):
        loss_sum = 0
        for t in range(self.num_steps):  # calculate loss for all t
            loss = self.calc_loss(
                observed_data, cond_mask, observed_mask, side_info, itp_info, is_train,revin_ref, set_t=t
            )
            loss_sum += loss.detach()
        return loss_sum / self.num_steps

    def calc_loss(
        self, observed_data, cond_mask, observed_mask, side_info, itp_info, is_train,revin_ref, set_t=-1
    ):
        ...
        predicted = self.diffmodel(total_input, side_info, t, itp_info, cond_mask) #(B,K,L)
        predicted = self.revin_layer(predicted,revin_ref,mode='denorm')
    ...
    def set_input_to_diffmodel(self, noisy_data, observed_data, cond_mask):
        ...
        return total_input
    def impute(self, observed_data, cond_mask, side_info, n_samples, itp_info,revin_ref):
        B, K, L = observed_data.shape
    ...
    def forward(self, batch, is_train=1):
        ...
        #coeffs (B,K,L)
        revin_ref = coeffs.clone().permute(0,2,1)
        observed_data.permute(0,2,1)  #(B,L,K)
        observed_data = self.revin_layer(observed_data,revin_ref,mode='norm') #(B,L,K) 
        observed_data.permute(0,2,1)  #(B,K,L)
        side_info = self.get_side_info(observed_tp, cond_mask)
        itp_info = None
        if self.use_guide:
            itp_info = coeffs.permute(0,2,1)  #(B,L,K)
            itp_info = self.revin_layer(itp_info,revin_ref,mode='norm') #(B,L,K) 
            itp_info = itp_info.permute(0,2,1)  #(B,K,L)
            itp_info = itp_info.unsqueeze(1)
        

        loss_func = self.calc_loss if is_train == 1 else self.calc_loss_valid
        output = loss_func(observed_data, cond_mask, observed_mask, side_info, itp_info, is_train,revin_ref)
        return output
    ...
    def evaluate(self, batch, n_samples):
        ...
        with torch.no_grad():
            ...
            samples = self.impute(observed_data, cond_mask, side_info, n_samples, itp_info,revin_ref)
